...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	// Bot karakterler i�in sald�r� kontrol�
	if (pkChr->IsBotCharacter())
	{
		// Bot'lar metin ta�lar�na her zaman sald�rabilir
		if (pkVictim->IsStone())
			return true;
		
		// Bot'lar d��man imparatorluk botlar�na sald�rabilir
		if (pkVictim->IsBotCharacter() && pkChr->GetEmpire() != pkVictim->GetEmpire())
			return true;
		
		// Bot'lar d��man imparatorluk oyuncular�na sald�rabilir
		if (pkVictim->IsPC() && !pkVictim->IsBotCharacter() && pkChr->GetEmpire() != pkVictim->GetEmpire())
			return true;
		
		// Bot'lar normal moblara sald�rabilir
	...
	else
	{
		switch( pkChr->GetMountVnum() )
		{
			...
			case 40003:		// Standing Mount: Turbo S�rf Tahtas�
			case 40004:		// Standing Mount: Wukong'un F�rt�nas�
			case 40005:		// Standing Mount: Wukong'un G�rlemesi
#endif
				break;
