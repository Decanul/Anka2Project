...
void CInputMain::TargetInfoLoad(LPCHARACTER ch, const char* c_pData)
{
	...
	// Ger�ek oyuncu kontrol�
	if (!ch->IsPC())
		return;

	// Oyuncular� hari� tut (sadece yarat�klar ve ta�lar i�in �al���r)
	if (m_pkChrTarget->IsPC())
		return;

#ifdef ENABLE_BOT_PLAYER
    // Bot karakterleri tamamen hari� tut
...
int CInputMain::Whisper(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	{
#ifdef ENABLE_BOT_PLAYER
		// Bot karakter kontrol� - e�er hedef bir bot ise, "xxx f�s�ldamay� bloklad�" mesaj�n� whisper paketi ile g�nder
		if (pkChr && (pkChr->IsBotCharacter() || CBotCharacterManager::instance().IsBotCharacter(pinfo->szNameTo)))
		{
			if (ch->GetDesc())
			{
				char blockedMsg[CHAT_MAX_LEN + 1];
				snprintf(blockedMsg, sizeof(blockedMsg), "%s f�s�ldamay� bloklad�.", pinfo->szNameTo);
...
int CInputMain::ItemsPickup(LPCHARACTER ch, const char * c_pData, size_t uiBytes)
{
	...
	// Integer overflow kontrol�: count de�erini kontrol et
	if (p->count == 0 || p->count > 1000)
		return -1;
	
	const int iExtraLen = p->count * sizeof(DWORD);

	// Integer overflow kontrol�: iExtraLen negatif veya �ok b�y�k olamaz
...
void CInputMain::SetSkillColor(LPCHARACTER ch, const char* pcData)
{
	...
	// Array bounds kontrol�: bSkillSlot de�eri ge�erli aral�kta olmal�
	...
	// Array bounds kontrol� ile g�venli eri�im (zaten yukar�da kontrol edildi ama ekstra g�venlik i�in)
	if (CGPacket->bSkillSlot < (ESkillColorLength::MAX_SKILL_COUNT + ESkillColorLength::MAX_BUFF_COUNT))
	{
		// �kinci boyut kontrol�: MAX_EFFECT_COUNT kontrol� (5 de�er yaz�l�yor, index 0-4)
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	// G�venlik: SwitchBot'tan depoya transfer engeli (Item Duplication �nleme)
#ifdef ENABLE_RENEWAL_SWITCHBOT
...
void CInputMain::SafeboxCheckout(LPCHARACTER ch, const char * c_pData, bool bMall)
{
	...
	// G�venlik: Depodan SwitchBot'a transfer engeli (Item Duplication �nleme)
#ifdef ENABLE_RENEWAL_SWITCHBOT
	if (p->ItemPos.IsSwitchbotPosition())
	{
		...
		case GUILD_SUBHEADER_CG_WITHDRAW_MONEY:
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Yang miktar�n maksimum seviyeye ula�m��.");
					return SubPacketLen;
				}
#endif
...
#ifdef ENABLE_BOT_PLAYER
				// Bot karakter kontrol� - e�er hedef bir bot ise, lonca daveti engelleme mesaj�n� g�nder
...
, p->seal_of_god
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Y�lan Tap�na�� Demircisi'nden �d�l� yaln�zca her 24 saatte bir alabilirsin.");
				}
			}
			else
			{
